...
int main(int argc,char **argv)
{
  ...
  TFile *file = new TFile("PythiaOutput_NoFSR.root","RECREATE");
  TH1F *zttpos= new TH1F("zttpos","pos",150,60,120);
  TH1F *zttneg= new TH1F("zttneg","neg",150,60,120);
	Log::SummaryAtExit();
...
*/
	...
	for (int iEvent = 0; iEvent < NumberOfEvents; ++iEvent)
	{
		...
		delete t_event;
		//	std::cout<<"Helicities  "<< Tauola::getHelPlus() << std::endl;
		...
		//		if(Z.M() > 90.9)
		  {
		  zttpos->Fill(Z.M(),HelWeightPlus);
		  zttneg->Fill(Z.M(),HelWeightMinus);
		}
		//clean up HepMC event
		delete HepMCEvt;
	}
